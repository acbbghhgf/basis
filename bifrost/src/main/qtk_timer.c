...
{
	struct timer_node *node = (struct timer_node *)qtk_malloc(sizeof(*node)+sz);
...
static inline void dispatch_list(struct timer_node *current)
{
    do {
        ...
        qtk_message_t message;
        message.source = 0;
        message.session = event->session;
        message.data = NULL;
        message.sz = (size_t)QTK_TIMER << MESSAGE_TYPE_SHIFT;

        qtk_context_push(event->handle, &message);
...
{
	struct timer *r = (struct timer *)qtk_malloc(sizeof(struct timer));
	memset(r, 0, sizeof(*r));
...
static void timer_destroy_timer(struct timer *t)
{
    qtk_free(t);
}
...
int qtk_timeout(uint32_t handle, int time, int session)
{
    if (time <= 0) {
        ...
        message.data = NULL;
        message.sz = (size_t)QTK_RET << MESSAGE_TYPE_SHIFT;

        if (qtk_context_push(handle, &message)) {
            return -1;
        }
    } else {
