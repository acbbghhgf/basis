...
struct qtk_socket_server {
    ...
    int lis_cap;
    wtk_thread_t thread;
    qtk_abstruct_hashtable_t *parsers;
    unsigned run:1;
};
...
struct qtk_parser* qtk_ss_get_parser(qtk_socket_server_t *ss, const char *proto);
...
int qtk_ss_add_socket(qtk_socket_server_t *ss, struct qtk_socket *socket, const char *proto);
