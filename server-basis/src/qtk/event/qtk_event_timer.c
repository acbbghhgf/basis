...
static void* qtk_event_timer_add_item(qtk_event_timer_t *timer, void *node) {
    void *handle;
    wtk_lock_lock(&timer->lock);
    handle = qtk_bheap_insert_node(timer->timer_q, node);
...
int qtk_event_timer_process(qtk_event_timer_t *timer) {
    ...
    while (qtk_bheap_head(timer->timer_q)) {
        qtk_bheap_node_t *n;
        wtk_lock_lock(&timer->lock);
        n = qtk_bheap_head(timer->timer_q);
        ti = n->data;
        if (ti->key > (long long)timer->time.wtk_cached_time) {
            wtk_lock_unlock(&timer->lock);
            break;
        }
        qtk_bheap_remove_node(timer->timer_q, n);
        wtk_lock_unlock(&timer->lock);
        if (ti->handler(ti->data) == QTK_TIMER_AGAIN) {
            ti->key += ti->delay;
            qtk_event_timer_add_item(timer, n);
        } else {
            wtk_lock_lock(&timer->lock);
            qtk_timer_release_item(timer, ti);
            qtk_bheap_del_node(timer->timer_q, n);
            wtk_lock_unlock(&timer->lock);
        }
    }
end:
