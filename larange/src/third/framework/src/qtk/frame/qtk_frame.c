...
static wtk_log_t* qtk_frame_get_base_log(qtk_fwd_t *frame);
...
int qtk_fwd_init(qtk_fwd_t *h,qtk_conf_file_t *cf)
{
    ...
    if (qtk_uplayer_init(&h->uplayer, h)) goto end;
    if (cfg->send_mail) {
...
int qtk_fwd_run_daemon(qtk_fwd_t *s)
{
	...
	while (s->run) {
		pid = fork();
		if (pid == 0) {
			wtk_log_log(s->cf->log,"fork pid=%#x,count=%d.",getpid(),count);
			qtk_fwd_run_proc(s);
			break;
		} else if (pid > 0) {
...
int qtk_uplayer_init(qtk_sframe_module_t *uplayer, qtk_fwd_t *frame) {
    ...
    uplayer->handle = dlopen(tmp, RTLD_NOW | RTLD_GLOBAL);
...
qtk_sframe_msg_t *qtk_fwd_request_msg(qtk_fwd_t *frame, qtk_request_t *request) {
    qtk_sframe_msg_t *msg = qtk_frame_msg_new_base(frame, QTK_SFRAME_MSG_REQUEST, request->s->id);
...
qtk_sframe_msg_t *qtk_fwd_response_msg(qtk_fwd_t *frame, qtk_response_t *response) {
    qtk_sframe_msg_t *msg = qtk_frame_msg_new_base(frame, QTK_SFRAME_MSG_RESPONSE, response->s->id);
