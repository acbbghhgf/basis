...
static void Base64decode(char *bufplain, const char *bufcoded, int *len) {
    register const unsigned char *bufin;
    ...
    *(bufout++) = '\0';
    if (len)
        *len = bufout - (unsigned char *)bufplain - 1;
}
...
{
    if (Base64encode_len(dlen) > rlen) {
        return NULL;
    }
    Base64encode(result, data, dlen);
    return result;
}

char *qtk_base64url_decode(const char *data, char *result, int rlen, int *real_len)
{
    int r_len = Base64decode_len(data);
    if (r_len > rlen) {
        return NULL;
    }
    Base64decode(result, data, real_len);
    return result;
}
