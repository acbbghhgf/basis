...
static int _socket_read_handle(qtk_socket_t *s, qtk_event_t *ev) {
    ...
    while (s->state == SOCKET_CONNECTED) {
        ...
        assert(s->server && s->proto.parser);
        if (readed && s->server && s->proto.parser) {
            //qtk_sframe_method_t *method = &s->server->entry->frame->method;
            s->proto.parser->handler(s->proto.parser, (void*)s, buf, readed);
        }
...
int qtk_socket_reset(qtk_socket_t *s) {
    ...
    if (s->proto.parser) {
        s->proto.parser->reset_handler(s->proto.parser);
    }
    if (s->send_stk) {
...
static void qtk_socket_close_parser(qtk_socket_t *s) {
    if (s->proto.parser) {
        qtk_parser_delete(s->proto.parser);
        s->proto.parser = NULL;
    }
}
...
int qtk_socket_init(qtk_socket_t *s, int fd,
    ...
    if (qtk_ss_add_socket(ss, s, proto)) {
        return -1;
    }
    qtk_socket_init_event(s);
    s->websocket = 0;
...
int qtk_socket_init_ws(qtk_socket_t *s, qtk_entry_t *entry, struct lws *wsi) {
    s->fd = INVALID_FD;
    s->server = qtk_entry_get_worker(entry, wsi->tsi);
    s->proto.wsi = wsi;
    s->websocket = 1;
    return 0;
}
