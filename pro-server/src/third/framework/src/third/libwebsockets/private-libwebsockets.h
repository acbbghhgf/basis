...
#endif
    qtk_event_module_t *io_loop_qev;
...
#define lws_get_vh_protocol(vh, x) vh->protocols[x]
    ...
    LWS_EXTERN void
    lws_libqev_io(struct lws *wsi, int flags);
    LWS_EXTERN int
    lws_libqev_init_fd_table(struct lws_context *context);
    ...
    LWS_EXTERN void
    lws_libqev_accept(struct lws *new_wsi, lws_sock_file_fd_type desc);
