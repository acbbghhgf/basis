...
static int qtk_http_parser_claim_request(qtk_http_parser_t *p, qtk_socket_t *s) {
    if (!p->pack) {
        ...
        qtk_request_init((qtk_request_t*)p->pack, s->id);
    }
    return 0;
}
...
static int qtk_http_parser_claim_response(qtk_http_parser_t *p, qtk_socket_t *s) {
    if (!p->pack) {
        ...
        if (handler->empty(handler->layer, p)) return -1;
        if (s) {
            qtk_response_init((qtk_response_t*)p->pack, s->id);
        } else {
            qtk_response_init((qtk_response_t*)p->pack, -1);
        }
    }
    return 0;
}
...
qtk_http_parser_t* qtk_http_parser_new() {
    ...
    p->clone_handler = (qtk_parser_clone_handler)qtk_http_parser_clone;
