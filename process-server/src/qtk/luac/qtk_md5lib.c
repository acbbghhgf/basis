 ...
 *  @return  A 128-bit hash string.
 */
static int lmd5hex (lua_State *L) {
...
static int lmd5_new(lua_State *L) {
    const char *p = NULL;
    size_t sz = 0;
    qtk_md5_t *md5;
    if (lua_gettop(L) == 1) {
        p = luaL_checklstring(L, 1, &sz);
    }
    md5 = lua_newuserdata(L, sizeof(*md5));
    qtk_md5_init(md5, p, sz);
    luaL_getmetatable(L, "md5lib");
...
static int lmd5_update(lua_State *L) {
    const char *p = NULL;
    size_t sz = 0;
    qtk_md5_t *md5 = lua_touserdata(L, 1);
    p = luaL_checklstring(L, 2, &sz);
    qtk_md5_update(md5, p, sz);
    return 0;
}
...
static int lmd5_hexdigest(lua_State *L) {
    unsigned char buff[16];
    char tmp[33];
    int i;
    qtk_md5_t *md5 = lua_touserdata(L, 1);
    qtk_md5_digest(md5, (char*)buff);
    for (i=0; i<sizeof(buff); i++)
        sprintf(tmp+i*2, "%02x", buff[i]);
    lua_pushstring(L, tmp);
...
static struct luaL_Reg md5_meta[] = {
    {"update", lmd5_update},
    {"digest", lmd5_digest},
    {"hexdigest", lmd5_hexdigest},
    /* {"__gc", lmd5_gc}, */
    {NULL, NULL},
};
...
int luaopen_md5_core (lua_State *L) {
    luaL_checkversion(L);
    assert(0 == luaL_getmetatable(L, "md5lib"));
    luaL_newmetatable(L, "md5lib");
    lua_pushvalue(L, -1);
