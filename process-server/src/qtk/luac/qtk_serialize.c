...
blk_alloc(void) {
	struct block *b = wtk_malloc(sizeof(struct block));
	b->next = NULL;
	return b;
}
...
LUAMOD_API int qtk_lua_unpack(lua_State *L) {
...
LUAMOD_API int qtk_lua_pack(lua_State *L) {
	struct block temp;
	temp.next = NULL;
