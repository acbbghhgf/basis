...
void qtk_md5_init(qtk_md5_t *md5, const char *msg, long len) {
    inic_digest(md5->d);
    md5->len = 0;
    md5->total_len = 0;
    qtk_md5_update(md5, msg, len);
}
...
void qtk_md5_update(qtk_md5_t *m, const char *msg, long len) {
    ...
    if (len + m->len >= 64) {
        ...
        memcpy(m->buf + m->len, msg, ncopy);
        msg += ncopy;
        len -= ncopy;
        d_old[0]=d[0]; d_old[1]=d[1]; d_old[2]=d[2]; d_old[3]=d[3];
        converte(wbuff, m->buf, numbytes, 0);
        digest(wbuff, d);
        d[0]+=d_old[0]; d[1]+=d_old[1]; d[2]+=d_old[2]; d[3]+=d_old[3];
    } else {
        memcpy(m->buf + m->len, msg, len);
        m->len += len;
        return;
    }
    while (len >= 64) {
...
void qtk_md5_digest(qtk_md5_t *m, char *output) {
    ...
    while (status != 2) {
        WORD32 d_old[4];
        WORD32 wbuff[16];
        int numbytes = m->len - i;
        /*salva os valores do vetor digest*/
        d_old[0]=d[0]; d_old[1]=d[1]; d_old[2]=d[2]; d_old[3]=d[3];
        status = converte(wbuff, m->buf+i, numbytes, status);
        if (status == 2) put_length(wbuff, m->total_len);
        digest(wbuff, d);
        d[0]+=d_old[0]; d[1]+=d_old[1]; d[2]+=d_old[2]; d[3]+=d_old[3];
        i += numbytes;
    }
